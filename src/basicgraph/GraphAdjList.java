...
public class GraphAdjList extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		List<Integer> firstNeighbors = adjListsMap.get(v);
		if (firstNeighbors != null) {
			for (int w : firstNeighbors) {
				List<Integer> secondNeighbors = adjListsMap.get(w);
				if (secondNeighbors != null) {
					for (int u : secondNeighbors) {
						result.add(u);
					}
				}
			}
